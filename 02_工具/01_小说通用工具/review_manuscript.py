...
CHECKLIST_PASS2 = """第二遍·带参照（细纲 + 世界基本法则 + 主角档案 + 禁用词表 + 本章开篇状态[若提供]）——查穿帮：
...
6b. **跨章事实穿帮**（若提供了"本章开篇状态"）：正文里提到的具体人名/事件/因果，逐一对照"本章开篇状态"里各对象的【关系概述】【当前心境】【隐患·未解代价】等字段——这些字段是前几章正文折叠出的浓缩事实。正文说法与这些字段矛盾（例如把甲事件的因果安到乙角色头上），一律标 `[跨章矛盾]`，并引用开篇状态里的原句作为反证
6c. **意象/俗语未铺垫**：对话或内心独白里但凡用了比喻、俗语、类比（尤其角色"忽然说"的警句式台词），自问——看懂这句话是否要求读者已经知道一件本书正文从未出现过的世界观设定或事件（例如拿"矿工能成仙"打比方，但全书正文没有任何一处出现过"仙人/成仙/机缘"这类铺垫）？如果这个比喻的两端要靠读者自己脑补桥梁才能连起来，标 `[意象未铺垫]`，并指出比喻里到底是哪个预设没有正文支撑。这类问题即使细纲本身就是这么写的（细纲的示例台词是方向指引，不是逐字稿）也要标出来，不因为"细纲也这么写"就放过
6d. **道具/人物状态跨章连续性**（对照"细纲"里自带的【0. 上下文滑动窗口】——上章末尾摘要 + 上章结尾原文最后 500 字，这份参照本来就在你手上，不是额外材料）：本章正文里每一件具名道具（谁拿着、在哪、还剩多少）、每个角色的伤势/所在地/正在做的事，是否跟滑动窗口摘录的上章结尾状态衔接得上。这类细节多半**不在**状态系统里（一次性道具/群像不建状态对象，"本章开篇状态"查不到），滑动窗口是唯一记录它们上一章下落的地方，冷读必须主动去对，不能默认"正文写的就是对的"。矛盾一律标 `[跨章连续性]`，引用滑动窗口原句反证（ch0005 教训：滑动窗口已写酒葫芦交到了主角手上，正文却写配角"从腰间扯下"同一只葫芦重新甩过去——这份参照当时就在评审器手里，只是清单没让它去查）
7. 细纲漏写（反向覆盖）：细纲明确要求的东西，正文里找不到对应文字落地 → 逐条标出（写"细纲要求 X → 正文没有")。重点查：
   - 场景内容简述里点名的关键 beat / 转折 / 冲突
   - 「涉及资源」「涉及伏笔」列的每一项（尤其伏笔的埋设方式：发热/共鸣/异动…有没有真写进正文）
   - 「场景钩子」的核心张力
   - 主角做某件事的**动机与前因**（云端模型最爱压掉连接性交代，只留动作）
   - 细纲指定的展示项：底层视角细节 ×N、谨慎行为、内心评估、年龄锚定、道义埋设等"""

CHECKLIST_OUTLINE1 = """第一遍·无任何参照——只读这份细纲本身：
1. 场景功能是否重复（推进/揭示/情感/转折 四类，别两场同功能）
2. 是否连续两个「转折」场景（节奏禁忌）
3. 单场景是否畸重（同质连续 >800 字要切分）
4. 本章特有的数值/计量/经济**变化**是否算清（单位、折算、克扣基数与结果）
5. 出场对象清单是否完整（@财务/@物品/@关系 别漏）
6. 前后自相矛盾之处——**重点核对【0. 上下文滑动窗口】（上章结尾摘要/原文）与后面【场景要点】里同一批道具/人物的状态是否一致**：上章结尾交代某物品已转手/某人已离场/某伤势已处理，后面场景若又写成"首次交给/刚好在场/伤势未处理"，就是同一份细纲自己两个区块打架（ch0005 教训：滑动窗口已写酒葫芦交给了苏砚，场景要点却又写周莽"甩过"酒葫芦，两处矛盾一路抄进了正文才被发现）
7. 瘦身：场景「内容简述」是否超 120 字；细纲是否整段复述了通用规则/全书口径原文（经济换算基准、人设红线、异宝档位、禁用词、结构节奏通则——这些应写指针「见红线包 §X」，不重抄）→ 标出并建议改成指针"""
...
{"findings":[{"severity":"high|med|low","kind":"矛盾|不闭合|物理|未回收|人物|脑补|漏写|穿帮|世界规则|经济|节奏","where":"第几段或原文引文（20字内）","problem":"一句话说清问题","fix_hint":"改的方向，不强制"}]}
...
def _resolve_targets(args):
    ...
    if args.chapter_dir:
        ...
        if mode == "outline":
            ...
            ref = {"细纲": outline}
            # 本章开篇状态是前几章正文折叠出的浓缩事实（关系概述/心境/隐患字段），
            # 也是当初喂给写正文的模型的同一份"是什么真实发生过"的依据——
            # 冷读评审器不带这份参照，就查不出"云端模型凭空编一个具体人名/因果
            # 当跨章事实"这类幻觉（ch0005"你替老刘家那小子扛了"的教训：细纲没
            # 写错，是正文自由发挥出来的，没有跨章事实参照的评审器根本看不出来）
            opener = chdir / "02_状态" / "00_开篇状态.md"
            if opener.exists():
                ref["本章开篇状态"] = opener
    else:
