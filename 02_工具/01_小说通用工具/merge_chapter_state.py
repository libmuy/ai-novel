...
把某一章的 `01_状态履历.md` 折叠进小说的唯一权威状态存储 `05_工作区/02_状态/01_最新状态/`
（每对象一文件、按类目分层的目录树）。

模型
----
- `05_工作区/02_状态/01_最新状态/` == 冻结基线 `05_工作区/02_状态/00_基线状态/` ⊕（按章节路径顺序
  折叠全部 `01_状态履历.md`）。
...
----
    python3 02_工具/01_小说通用工具/merge_chapter_state.py --chapter-dir <章目录> [选项]
      ...
      --chapter-dir PATH   必填，要并入 05_工作区/02_状态/01_最新状态/ 的那一章
...
def main():
    ...
    except Exception as e:  # noqa: BLE001 —— 开篇状态刷新失败不应中断合并
