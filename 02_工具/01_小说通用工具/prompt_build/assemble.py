...
_EVENT_TPL = {
    # 注意：不能用裸「冲突」做键——节拍表「核心事件类型」里「冲突/谈判」「危机/冲突」
    # 也含这两个字，会被误判成「冲突/战斗」而错拉战斗结算模板（章0006 教训：
    # 该章 必用模板=00_通用写作规则、核心事件类型=冲突/谈判，却被内联了战斗结算模板）。
    # 「战斗」一词足以唯一命中「冲突/战斗」这一类，勿再加回「冲突」。
...
**正文**：第一行单独写一级标题 `# 章名`——章名**逐字照抄本章细纲【基础信息】「章名」字段**，不得自拟、不得写成「第N章 / 章NNNN」这类编号式；标题后空一行再接散文，按细纲的场次顺序连续写出，场次之间空一行分隔，场次不加编号标题、不加旁白说明。
...
def _outline_task(beat: Optional[dict]) -> str:
    ...
    items = [
        "场景切分 2~5 场，每场写明**预算字数 / 功能（推进·转折·情感·揭示）/ 内容要点 / 场景钩子**；"
        "四类功能不重复，禁止连续两个「转折」场；单场同质内容不超过 800 字。",
        "「## 出场对象」表**必填且完整**——本章登场、被提及并影响本章、或状态被改动的对象逐个列出，"
        "含物品 / 财务 / 关系类状态对象。此表是章后状态对账的取数依据，漏一个就会在对账时报警。",
        "**本章特有**的数值 / 计量 / 经济**变化**必须在细纲内算清（单位、折算率、基数、结果算得通），"
        "不得留给正文临场编数字；但全书恒定的换算基准（如「N 两废料折一枚」）、主角人设红线、异宝档位、"
        "禁用词等**写指针「见红线包 §X」，不重抄正文**——正文提示词会另行内联红线包，抄两遍只会口径漂移。",
        "细纲是场景骨架、不是迷你提示词：场景「内容简述」≤120 字、只写本场事件；场景下的红线提醒用一句指针，不复述规则原文。",
        "伏笔的埋设 / 推进 / 回收只能用【已有数据】里已登记的编号，**禁止现编**；本章不涉及就写「无」。",
    ]
    # 只有本章确实拉了事件卡模板（战斗/突破/事件与感悟）才提这句——「核心事件类型」
    # 有值（如「冲突/谈判」）不等于有对应事件卡模板，别让编号列表指向一个提示词里根本
    # 不存在的模板（章0006 教训，见 `_EVENT_TPL` 旁注）。
    if kind and _event_templates_from_beat(beat):
        items.append(f"本章核心事件类型为「{kind}」，须按【必读模板】对应的事件卡模板补齐要素区块。")
    if hook:
        items.append(f"章末钩子类型为「{hook}」，按此设计，不得改成别的强度。")
    items.append("拿不准、或【已有数据】不足以判定的，列进「待确认清单」交作者裁决，**不要自行编造**。")
