...
把旧模型的状态布局迁移到新模型：
  - 旧：04_全局状态/ 下 5 个扁平分类文件（01_角色状态.md … 05_世界状态.md）
        + 每章 03_本章初始状态.md（链式）+ 卷末 99_卷末状态快照.md
  - 新：04_全局状态/ 每对象一文件的目录树（唯一权威）
...
`04_全局状态/` == 冻结基线 `05_工作区/00_全局/00_基线状态/`
...
def main():
    ...
    gdir = os.path.join(novel, "04_全局状态")
    ...
    plan += [f"删扁平文件   -> 04_全局状态/{f}" for f in flat_present]
